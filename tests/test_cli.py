...
def test_viewer_help_exposes_annotation_output() -> None:
    result = run_cli("viewer", "--help")
    assert result.returncode == 0
    assert "--annotation-output" in result.stdout


def test_viewer_help_explains_interactive_annotation_requirement() -> None:
    result = run_cli("viewer", "--help")
    assert result.returncode == 0
    assert "interactive mode" in result.stdout
    assert "does not open Qt" in result.stdout
...
def test_viewer_argument_failures_use_argparse_exit_code(tmp_path: Path) -> None:
    ...
    for result in (no_root, no_active_frames, no_annotations, invalid_manifest):
        assert result.returncode == 2
    ...
    assert "--annotation-output is required in interactive mode" in no_annotations.stderr
